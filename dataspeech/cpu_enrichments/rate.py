...
transducer = make_g2p('dan', 'dan-ipa')
...
# backend = EspeakBackend('da', with_stress=True)
...
#         phonemes_list = []
#         for text, audio in zip(batch[text_column_name], batch[audio_column_name]):
#             phonemes = phonemize(text, language='da', backend='espeak', with_stress=True)
...
#         batch["phonemes"] = phonemes_list
#     else:
#         phonemes = phonemize(batch[text_column_name], language='da', backend='espeak', with_stress=True)
